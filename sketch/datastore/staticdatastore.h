...
#define MACRO_GET_1(str, i) (sizeof(str) - 2 > (i) ? str[(i)] : 0)
...
#define MACRO_GET_64(str, i)                           \
  MACRO_GET_16(str, i + 0), MACRO_GET_16(str, i + 16), \
      MACRO_GET_16(str, i + 32), MACRO_GET_16(str, i + 48)

#define CT_STR(str) string_t<MACRO_GET_64(#str, 1), 0>

template <typename ValueType, typename KeyType, KeyType keyTag_,
          typename StaticKeyStr>
class KeyDataPair {
 public:
...
enum class KeyTag : size_t {
  ...
  //
  KeyTagSize  // do not remove this last value, the template meta function
              // skiping the last element (this), if you remove this value you
              // have to rewrite the conditions
};
...
using StaticDataStore =
    std::tuple<Entry<int, KeyTag::AntDirection, CT_STR("AntDirection")>,
...
static_assert(size_t(KeyTag::KeyTagSize) == ValueKeyStoreSize,
              "StaticDataStore has less or more KeyDataPair than KeyTag");
...
struct TupleCheckOrder {
  static void checkOrder(const Tuple& t) {
    ...
    // using CurrentType = typename std::tuple_element<currentN,typename
    // std::decay<Tuple>::type>::type::Type;

    static_assert(
        size_t(std::tuple_element<
               currentN, typename std::decay<Tuple>::type>::type::keyTag) ==
            currentN,
        "Order of StaticDataStore type arguments is not in sync with KeyTag "
        "order");
...
struct TupleCheckOrder<Tuple, 1> {
  static void checkOrder(const Tuple&) {
    ...
    using CurrentType = typename std::tuple_element<
        currentN, typename std::decay<Tuple>::type>::type::Type;
    (void)CurrentType();
  }
};
...
class StaticDataStoreException : public std::runtime_error {
 public:
  using std::runtime_error::runtime_error;
};
class StaticDataStoreKeyNotFound : public StaticDataStoreException {
 public:
  using StaticDataStoreException::StaticDataStoreException;
};
class StaticDataStoreTypIsNotConvertiable : public StaticDataStoreException,
                                            boost::bad_any_cast {
 public:
  using StaticDataStoreException::StaticDataStoreException;
};
...
struct TupleAnySetter<Tuple, 0> {
  static void set(Tuple&, std::string const& key, boost::any) {
    throw StaticDataStoreKeyNotFound("Key \"" + key +
                                     "\" is not known in DataStore");
  }
};
...
template <typename ReturnType, typename CurrentType,
          bool is_convertible =
              std::is_convertible<CurrentType, ReturnType>::value>
...
struct TupleGetter {
  static ReturnType getByKeyStr(Tuple& t, std::string const& key) {
    ...
    using CurrentType = typename std::tuple_element<
        currentN, typename std::decay<Tuple>::type>::type::Type;
    if (key == std::get<currentN>(t).keyStr())
      return EnsureIsConvertiable<ReturnType, CurrentType>::forward(
          std::get<currentN>(t).value);
    else
...
ReturnType get([[gnu::unused]] std::tuple<Args...>& t, std::string const& key) {
  return detail::TupleGetter<ReturnType, decltype(t),
                             sizeof...(Args)>::getByKeyStr(t, key);
}
}  // namespace unsafe
