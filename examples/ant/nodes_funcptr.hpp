...
class FactorySimpleHash {
  ...
  static std::array<NodeCreateFunction, kMaxHash> makeHashToNode(
      NodeDefArray nodeDefs) {
    std::array<NodeCreateFunction, kMaxHash> creatFunctionLUT{nullptr};

    for (auto aNodeDef : nodeDefs) {
      auto hash = simpleHash<kMaxHash>(std::begin(aNodeDef.name),
                                       std::end(aNodeDef.name));

      creatFunctionLUT[hash] = [aNodeDef,
                                nodeDefs](CursorType &tokenCursor) -> NodeType {
        auto node = aNodeDef;
        for (auto &child : node.children) {
          tokenCursor.next();
          child = FactorySimpleHash<NodeType, CursorType, kMaxHash>::factory(
              tokenCursor, nodeDefs);
        }
        return node;
      };
    }
  ...
  static NodeCreateFunction getCreateFuntion(TokenType token,
                                             NodeDefArray nodeDefs) {
    static std::array<NodeCreateFunction, kMaxHash> creatFunctionLUT =
        makeHashToNode(nodeDefs);
